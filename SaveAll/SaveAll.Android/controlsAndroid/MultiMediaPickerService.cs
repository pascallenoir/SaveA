...
namespace SaveAll.Droid.controlsAndroid
{
    public class MultiMediaPickerService : IMultiMediaPickerService
    {
        ...
        public event EventHandler<Patrimoine> OnMediaPicked;
        public event EventHandler<IList<Patrimoine>> OnMediaPickedCompleted;

        TaskCompletionSource<IList<Patrimoine>> mediaPickedTcs;

        public void OnActivityResult(int requestCode, Result resultCode, Intent data)
        {
            ObservableCollection<Patrimoine> mediaPicked = null;

            if (requestCode == MultiPickerResultCode)
            {
                if (resultCode == Result.Ok)
                {
                    mediaPicked = new ObservableCollection<Patrimoine>();
                    if (data != null)
                    {
        ...
        public async Task<IList<Patrimoine>> PickPhotosAsync()
        {
            return await PickMediaAsync("image/*", "Select Images", MultiPickerResultCode);
        }

        public async Task<IList<Patrimoine>> PickVideosAsync()
        {
            return await PickMediaAsync("video/*", "Select Videos", MultiPickerResultCode);
        }

        async Task<IList<Patrimoine>> PickMediaAsync(string type, string title, int resultCode)
        {

            mediaPickedTcs = new TaskCompletionSource<IList<Patrimoine>>();
